...
(function() {
  ...
  activateColor = function($active) {
    var $blackout, $color, $row, $shades, $widget, hex, j, len, name, ref;
    cancelColor();
    $widget = $('.color-widget');
    $blackout = $widget.find('.blackout');
  ...
  buildWidget = function() {
    var $back, $blackout, $color, $row, $widget, columnNumber, hex, i, index, j, k, len, name, names, nextRow, rowNumber, rows;
    $widget = $('.color-widget');
    ...
    for (index = k = 0, len = names.length; k < len; index = ++k) {
      name = names[index];
    ...
    return $widget.find('.color').click(function(e) {
      ...
      if ($active.hasClass('active')) {
		...
		var msg = $(e.currentTarget).css('background-color');
		send(msg);
  ...
  colors = {
	...
	DarkSlateGray: ['#c7dede', '#aecfcf', '#a2c8c8', '#96c0c0', '#7db2b2', '#71aaaa', '#64a3a3', '#5b9999', '#548c8c', '#4c8080', '#457474', '#3e6868', '#365b5b', '#2f4f4f'],
...
$(document).on("click",".color", function () {
	var msg = $(this).css('background-color');
	send(msg);
});